/*
 *  [2015] - [2016] Codenvy, S.A.
 *  All Rights Reserved.
 *
 * NOTICE:  All information contained herein is, and remains
 * the property of Codenvy S.A. and its suppliers,
 * if any.  The intellectual and technical concepts contained
 * herein are proprietary to Codenvy S.A.
 * and its suppliers and may be covered by U.S. and Foreign Patents,
 * patents in process, and are protected by trade secret or copyright law.
 * Dissemination of this information or reproduction of this material
 * is strictly forbidden unless prior written permission is obtained
 * from Codenvy S.A..
 */
'use strict';

/**
 * @ngdoc directive
 * @name teams.workspaces:ListTeamWorkspaces
 * @restrict E
 * @element
 *
 * @description
 * `<list-team-workspaces team="ctrl.team"></list-team-workspaces>` for displaying list of workspaces
 *
 * @usage
 *   <list-team-workspaces team="ctrl.team"></list-team-workspaces>
 *
 * @author Ann Shumilova
 */
export class ListTeamWorkspaces implements ng.IDirective {

  restrict = 'E';
  templateUrl = 'app/teams/team-details/team-workspaces/list-team-workspaces.html';

  controller = 'ListTeamWorkspacesController';
  controllerAs = 'listTeamWorkspacesController';
  bindToController = true;

  scope = {
    team: '='
  };

  constructor () {
  }
}
